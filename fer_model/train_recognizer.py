...
from pyimagesearch.preprocessing import imagetoarraypreprocessor as image2arrayProc
...
from pyimagesearch.nn.conv import emotionvggnet as EVGG
...
trainGen = hdf5DG.HDF5DatasetGenerator (config.TRAIN_HDF5, config.BATCH_SIZE, aug=trainAug, preprocessors=[iap],
                                        classes=config.NUM_CLASSES)
valGen = hdf5DG.HDF5DatasetGenerator (config.VAL_HDF5, config.BATCH_SIZE, aug=valAug, preprocessors=[iap],
                                      classes=config.NUM_CLASSES)

if args["model"] is None:
    print ("[INFO] compiling model...")
    model = EVGG.EmotionVGGNet.build (width=48, height=48, depth=1, classes=config.NUM_CLASSES)
    opt = Adam (lr=1e-3)
